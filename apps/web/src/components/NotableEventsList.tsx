        ...
        <div key={row.id} className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:grid-cols-[120px_minmax(0,1fr)]">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="text-sm text-zinc-400">{formatEventTimestamp(row.localTimestamp)}</div>
              <div className="text-sm text-zinc-400">{row.cameraName}</div>
            </div>
            ...
            <div className="text-sm leading-6 text-zinc-300">{row.summary ?? row.analysisSummary ?? "No narrative available."}</div>
