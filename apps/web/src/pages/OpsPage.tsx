          ...
          <SectionCard title="Reporting Recently" subtitle="Camera freshness counts for the current slice.">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-xs uppercase tracking-[0.18em] text-zinc-400">Seen in 24h</div>
                <div className="mt-3 text-3xl font-semibold text-zinc-100">
                  {formatNumber(recentReporting.within24h)} <span className="text-lg text-zinc-400">/ {formatNumber(recentReporting.total)}</span>
                </div>
              </div>
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-xs uppercase tracking-[0.18em] text-zinc-400">Seen in 48h</div>
                <div className="mt-3 text-3xl font-semibold text-white">
                  {formatNumber(recentReporting.within48h)} <span className="text-lg text-zinc-400">/ {formatNumber(recentReporting.total)}</span>
                </div>
              </div>
            </div>
          </SectionCard>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {[
                ...
                <div key={String(label)} className="rounded-2xl bg-white/5 p-4">
                  ...
                  <div className="mt-2 text-sm text-zinc-300">Conversion {formatPercent(Number(conversionPct), 0)}</div>
                  <div className="mt-1 text-sm text-stone-200">Drop-off {formatPercent(Number(dropPct), 0)}</div>
                </div>
              ))}
            </div>
          </SectionCard>
        ...
        {overview ? (
          <SectionCard title="Telemetry Snapshot" subtitle="Power diagnostics without the hard-to-read line chart.">
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-xs uppercase tracking-[0.18em] text-zinc-400">Under 11.5v</div>
                <div className="mt-3 text-3xl font-semibold text-stone-200">
                  {formatNumber(overview.cameraHealth.filter((camera) => (camera.avgVoltage ?? Number.POSITIVE_INFINITY) < 11.5).length)}
                </div>
              </div>
              <div className="rounded-2xl bg-white/5 p-4">
                <div className="text-xs uppercase tracking-[0.18em] text-zinc-400">Lowest Voltage</div>
                <div className="mt-3 text-3xl font-semibold text-red-200">
              ...
              {lowestVoltageCameras.map((camera) => (
                <div key={camera.cameraName} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3">
                  <div>
                    ...
                    <div className="mt-1 text-sm text-zinc-400">{camera.alertReason ?? "Power reading available in current slice."}</div>
                  </div>
